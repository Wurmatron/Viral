...
public class ViralEventHandler {
    ...
    public void onLivingUpdate(LivingEvent.LivingUpdateEvent e) {
        ...
        if (status.status() == 1) {
            ...
            if (!(e.getEntityLiving() instanceof EntityAnimal)) {
                ...
                status.set(0);
                upgradeMaxHealth(e.getEntityLiving(), true);
            }
        }
    }
    ...
    public static void upgradeMaxHealth(EntityLivingBase entity, boolean undo) {
        double maxHP = entity.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).getBaseValue();
        if (undo)
            maxHP = maxHP / ConfigHandler.maxHPGrowth;
        else
            maxHP = maxHP * ConfigHandler.maxHPGrowth;
        // Make sure it does not overflow
        if (maxHP > Integer.MAX_VALUE - 1)
            maxHP = Integer.MAX_VALUE - 1;
        entity.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(maxHP);
    }

    public static void upgradeMaxHealth(EntityLivingBase entity) {
        upgradeMaxHealth(entity, true);
    }
}
